const Select = () => {
  return (
    <div className="flex flex-col gap-0.5 items-start self-stretch relative">
      ...
      <div className="flex justify-between items-start self-stretch rounded-lg border border-solid bg-zinc-200 border-zinc-300">
        ...
        <div className="flex gap-2 justify-center items-center w-10 bg-stone-50">
          <button
            ...
            className="flex gap-1 justify-center items-center p-2 rounded-sm cursor-pointer bg-zinc-200 hover:bg-zinc-300 transition-colors"
          >
            <svg
              width="24"
              height="24"
        ...
        <div className="absolute top-full left-0 right-0 z-10 mt-1 bg-white border border-zinc-300 rounded-lg shadow-lg max-h-40 overflow-y-auto">
          {options.map((option, index) => (
            <button
              ...
              className="w-full text-left px-3 py-2 text-sm text-zinc-900 hover:bg-zinc-100 first:rounded-t-lg last:rounded-b-lg"
            >
              {option}
            </button>
          ))}
